		...
		badge := `
			<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="113.5" height="28" role="img" aria-label="VIEWS: 3">
				...
				<g fill="#fff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" text-rendering="geometricPrecision" font-size="100">
					...
					<text id="counter" transform="scale(.1)" x="973.75" y="175" textLength="82.5" fill="#fff" font-weight="bold">{counter}</text>
				</g>
			</svg>
			`
		badge = strings.Replace(badge, "{counter}", fmt.Sprintf("%d", message), -1)
